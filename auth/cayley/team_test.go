...
func TestTeamRepository(t *testing.T) {
	...
	for i, team := range teams {
		testGetTeam(t, repo, team, fmt.Sprintf("get team %d", i))
	}
	...
	testGetTeam(t, repo, teams[0], "get team 0 after name update")
	...
	testGetTeam(t, repo, teams[1], "get team 1 after members update")

	// Update a team's permissions (canSee: +2 -1, canEdit: +2 -0)
	teams[1].CanSee = []int{1, 3, 4, 5}
	teams[1].CanEdit = []int{1, 3, 4, 5}
	testUpdateTeam(t, repo, teams[1])
	testGetTeam(t, repo, teams[1], "get team 1 after permissions update")
...
func testGetTeam(t *testing.T, repo auth.TeamRepository, team *auth.Team, name string) {
	retrieved, err := repo.Get(team.ID)
